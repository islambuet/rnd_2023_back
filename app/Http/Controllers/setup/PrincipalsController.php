...
class PrincipalsController extends RootController
{
    ...
    public function getItems(Request $request): JsonResponse
    {
        if ($this->permissions->action_0 == 1) {
            ...
            $query=DB::table(TABLE_PRINCIPALS);
    ...
    {
        if ($this->permissions->action_0 == 1) {
            $result = DB::table(TABLE_PRINCIPALS)->find($itemId);
            if (!$result) {
    ...
    public function saveItem(Request $request): JsonResponse
    {
        ...
        if ($itemId > 0) {
            $result = DB::table(TABLE_PRINCIPALS)->select(array_keys($validation_rule))->find($itemId);
            if (!$result) {
                return response()->json(['error' => 'ITEM_NOT_FOUND', 'messages' => __('Invalid Id ' . $itemId)]);
            }
        ...
        try {
            $time = Carbon::now();
            $dataHistory = [];
            $dataHistory['table_name'] = TABLE_PRINCIPALS;
            ...
            if ($itemId > 0) {
                ...
                DB::table(TABLE_PRINCIPALS)->where('id', $itemId)->update($itemNew);
                $dataHistory['table_id'] = $itemId;
                $dataHistory['action'] = DB_ACTION_EDIT;
            } else {
                $itemNew['created_by'] = $this->user->id;
                $itemNew['created_at'] = $time;
                $newId = DB::table(TABLE_PRINCIPALS)->insertGetId($itemNew);
