...
//        Input validation ends
    ...
    {
        if ($this->permissions->action_0 == 1) {
            $formInfo = DB::table(TABLE_TRIAL_REPORT_FORMS)->find($formId);
            if (!$formInfo) {
            ...
            $perPage = $request->input('perPage', 50);
            $query=DB::table(TABLE_TRIAL_REPORT_FORM_INPUTS);
    ...
    {
        if ($this->permissions->action_0 == 1) {
            $info = DB::table(TABLE_TRIAL_REPORT_FORM_INPUTS)->find($inputId);
            if (!$info) {
    ...
    public function saveItemInput(Request $request, $cropId,$formId): JsonResponse
    {
        ...
        //edit change checking
        if ($itemId > 0) {
            $result = DB::table(TABLE_TRIAL_REPORT_FORM_INPUTS)->select(array_keys($validation_rule))->find($itemId);
            if (!$result) {
...
//        Input validation ends
        ...
        try {
            $time = Carbon::now();
            $dataHistory = [];
            $dataHistory['table_name'] = TABLE_TRIAL_REPORT_FORM_INPUTS;
            $dataHistory['controller'] = (new \ReflectionClass(__CLASS__))->getShortName();
            $dataHistory['method'] = __FUNCTION__;
            $newId = $itemId;
            if ($itemId > 0) {
                $itemNew['updated_by'] = $this->user->id;
                $itemNew['updated_at'] = $time;
                DB::table(TABLE_TRIAL_REPORT_FORM_INPUTS)->where('id', $itemId)->update($itemNew);
                $dataHistory['table_id'] = $itemId;
                $dataHistory['action'] = DB_ACTION_EDIT;
            } else {
                $itemNew['created_by'] = $this->user->id;
                $itemNew['created_at'] = $time;
                $newId = DB::table(TABLE_TRIAL_REPORT_FORM_INPUTS)->insertGetId($itemNew);
                $dataHistory['table_id'] = $newId;
                $dataHistory['action'] = DB_ACTION_ADD;
            }
            unset($itemNew['updated_by'],$itemNew['created_by'],$itemNew['created_at'],$itemNew['updated_at']);
